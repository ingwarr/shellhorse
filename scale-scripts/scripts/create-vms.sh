...
apt-get -y update && apt-get --yes --force-yes install virtinst
...
function clone_vms {
  ...
  let "VM_COUNT = (MEM_TOTAL - 2097152) / VM_MEM"   
  for ((i=1; i<=VM_COUNT; i++)); do virt-clone -o gp -n gp-clone-$i --auto-clone; done
}
